package com.xth.systemcall.customview;
...
public class XGridView extends GridView {
    ...
    private Runnable mScrollRunbale = new Runnable() {
        ...
        public void run() {
            ...
            if (mMoveY > mUpScrollBorder) {
                scrollY = mSpeed;
                mHandler.postDelayed(mScrollRunbale, 25);//用Handler对象每个25ms调用一次mScrollRunbale，实现该线程定时25ms
            } else if (mMoveY < mDownScrollBorder) {
                scrollY = -mSpeed;
                mHandler.postDelayed(mScrollRunbale, 25);
            } else {
                scrollY = 0;
                mHandler.removeCallbacks(mScrollRunbale);//超过边界退出定时操作
            }
            smoothScrollBy(scrollY, 10);//移动scrollY，持续10ms
        }
    };
    ...
    public XGridView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        LogUtil.v("mStatusHeight:"+mStatusHeight);
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                //获取按下的position
                mDragPosition = pointToPosition(mDownX, mDownY);//根据坐标获取到当前的position
                ...
                mDragView = getChildAt(mDragPosition - getFirstVisiblePosition());//返回指定位置的视图
                if (mDragView == null) {
                    return super.dispatchTouchEvent(ev);
                }
                ...
                LogUtil.v("mPoint2ItemLeft:"+mPoint2ItemLeft+"-mPoint2ItemTop:"+mPoint2ItemTop);
                //计算GridView的left top 偏移量：原始距离 - 相对距离就是偏移量
                mOffset2Left = (int) ev.getRawX() - mDownX;
                mOffset2Top = (int) ev.getRawY() - mDownY;
                LogUtil.v("ev.getRawX():"+ev.getRawX()+"-mDownX:"+mDownX);
                LogUtil.v("ev.getRawY():"+ev.getRawY()+"-mDownY:"+mDownY);
                //获取GridView自动向下滚动的偏移量，小于这个值，DragGridView向下滚动
                mDownScrollBorder = getHeight() / 4;
                //获取GridView自动向上滚动的偏移量，大于这个值，DragGridView向上滚动
                mUpScrollBorder = getHeight() * 3 / 4;
                LogUtil.v("mDownScrollBorder:"+mDownScrollBorder+"-mUpScrollBorder:"+mUpScrollBorder);
                //开启视图缓存
    ...
    private void createDragView(Bitmap bitmap, int downX, int downY) {
        ...
        mLayoutParams.format = PixelFormat.TRANSLUCENT; //窗口透明
